//VPs code modified based on msac
#include "BaySAC.h"
#include "error.h"
#ifdef USE_LMFIT
#include "lmmin.h"
...
void BaySAC::init(int mode, cv::Size imSize, bool verbose)
{
	// Arguments
	__verbose = verbose;
...
void BaySAC::fillDataContainers(std::vector<std::vector<cv::Point> > &lineSegments)
{
	...
	// __Li = [l_00 l_01 l_02; l_10 l_11 l_12; l_20 l_21 l_22; ...]; 
	//where li=[l_i0;l_i1;l_i2]^T is li=an x bn; 
	...
	for (int i=0; i<numLines; i++)
	{
		...
		//get the mid point fill into __c 
		if(__mode == MODE_)
			__c = 0.5*(__a+__b);
		...
		else // __mode == MODE_ requires not to calibrate into the sphere
		{
			__an = __a;
			__bn = __b;
		}
...
void BaySAC::multipleVPEstimation(std::vector<std::vector<cv::Point> > &lineSegments, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<int> &numInliers, std::vector<cv::Mat> &vps, int numVps)
{	
	...
	for(int vpNum=0; vpNum < numVps; vpNum++)
	{
		...
		{					
			...
			{
				if(__verbose)
					printf("All line segments are inliers. End BaySAC at iteration %d.\n", iter);
				break;				
			}
		}
...
#ifdef USE_LMFIT
			else if(__mode == MODE_)
				estimate(__Li, __Lengths, __Mi, ind_CS, __N_I_best, __vp);	// Output __vp is calibrated
#endif
			else
				perror("ERROR: mode not supported, please use {LS, LIEB, }\n");
...
// RANSAC
void BaySAC::GetMinimalSampleSet(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &MSS, cv::Mat &vp)
{	
	int N = Li.rows;	
	...
	if(__mode == MODE_LS)
		...
		estimate(Li, Mi, Lengths, MSS, 2, vp);
#endif
	else
		perror("ERROR: mode not supported. Please use {LS, }\n");
}
float BaySAC::GetConsensusSet(int vpNum, cv::Mat& Li, cv::Mat& Lengths, cv::Mat& Mi, cv::Mat& vp, std::vector<float>& E, int* CS_counter)
{
	// Initialize the consensus set indices to an invalid value.
	...
	if (__mode == MODE_LS) {
		...
		J = error(vpNum, Li, Lengths, Mi, vp, E, CS_counter);
	}
#endif
	else {
		perror("ERROR: mode not supported, please use {LS, }\n");
		return J; // Return early in case of unsupported mode
	}

	// Assuming E contains the errors after calling error or errorLS
	// Find the indices of the three smallest errors and add them to the CS
	std::vector<std::pair<float, unsigned int>> errorWithIndex;
	for (unsigned int i = 0; i < E.size(); ++i) {
		errorWithIndex.emplace_back(E[i], i);
	}

	// Sort by error value
	std::partial_sort(errorWithIndex.begin(), errorWithIndex.begin() + std::min<unsigned int>(3, E.size()), errorWithIndex.end(),
		[](const std::pair<float, unsigned int>& a, const std::pair<float, unsigned int>& b) {
			return a.first < b.first;
		});

	// Add the top three to the consensus set
	for (unsigned int i = 0; i < std::min<unsigned int>(3, E.size()); ++i) {
		__CS_idx[errorWithIndex[i].second] = static_cast<int>(i); // Store index or some meaningful value
	}

	*CS_counter = std::min<unsigned int>(3, E.size()); // Update the counter with the number of elements added to CS
...
void BaySAC::estimateLS(cv::Mat &Li, cv::Mat &Lengths, std::vector<int> &set, int set_length, cv::Mat &vp)
{	
...
#ifdef USE_LMFIT
void BaySAC::estimate(cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, std::vector<int> &set, int set_length, cv::Mat &vp)
{
	if (set_length == __minimal_sample_set_dimension)
	{	
		// Just the cross product
		// DATA IS NOT CALIBRATED for MODE_
		cv::Mat ls0 = Mat(3,1,CV_32F);
		cv::Mat ls1 = Mat(3,1,CV_32F);
	...
	for(int t=0; t<numTheta; t++)
	{
		...
		{		
			...
			evaluate(paramTest, set_length, (const void*)&dataTest, fvecTest, infoTest);
	...
	lmmin(num_par, par, m_dat, &data, evaluate, &control, &status, lm_printout_std);
...
#endif
// Error functions
float BaySAC::errorLS(int vpNum, cv::Mat &Li, cv::Mat &vp, std::vector<float> &E, int *CS_counter)
{
	cv::Mat vn = vp;
...
#ifdef USE_LMFIT
float BaySAC::error(int vpNum, cv::Mat &Li, cv::Mat &Lengths, cv::Mat &Mi, cv::Mat &vp, std::vector<float> &E, int *CS_counter)
{
	...
	{ 		
		...
		di = distance(vn, lineSegment, lengthLineSegment, midPoint);
...
#endif
void BaySAC::drawCS(cv::Mat &im, std::vector<std::vector<std::vector<cv::Point> > > &lineSegmentsClusters, std::vector<cv::Mat> &vps)
{
